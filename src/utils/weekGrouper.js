 ...
 * @param {boolean} includeWeekends - Inclure les week-ends
 * @returns {Array<Object>} - Tableau de semaines avec leurs événements
 */
export function groupEventsByWeek(events, startDate, endDate, includeWeekends = false) {
  ...
  return weekStarts.map(weekStart => {
    ...
    const weekDays = getWeekDays(weekStart, includeWeekends)
 ...
 * @param {number} endHour - Heure de fin de la grille (défaut: 20)
 * @returns {number} - Position en minutes (minimum 0)
 */
export function calculateEventPosition(eventTime, startHour = 8, endHour = 20) {
  const hours = eventTime.getHours()
  const minutes = eventTime.getMinutes()
  const totalMinutes = (hours - startHour) * 60 + minutes
  const maxMinutes = (endHour - startHour) * 60
  // S'assurer que la position est dans la plage valide
  return Math.max(0, Math.min(totalMinutes, maxMinutes))
}
...
/**
 * Vérifie si un événement est dans la plage horaire visible
 * @param {Object} event - Événement avec start et end
 * @param {Date} day - Jour à vérifier
 * @param {Object} timeRange - Plage horaire {start, end} (défaut: 8h-20h)
 * @returns {boolean} - True si l'événement est visible
 */
export function isEventVisible(event, day, timeRange = { start: 8, end: 20 }) {
...
/**
 * Tronque un événement pour qu'il rentre dans la plage horaire
 * @param {Object} event - Événement avec start et end
 * @param {Date} day - Jour de référence
 * @param {Object} timeRange - Plage horaire {start, end} (défaut: 8h-20h)
 * @returns {Object} - Événement tronqué
 */
export function truncateEventToWorkHours(event, day, timeRange = { start: 8, end: 20 }) {
  ...
  // Si l'événement commence avant ce jour, le commencer à l'heure de début ce jour
  if (eventStart < dayStart) {
    eventStart = dayStart
  }

  // Si l'événement finit après ce jour, le finir à l'heure de fin ce jour
  if (eventEnd > dayEnd) {
    eventEnd = dayEnd
  }
